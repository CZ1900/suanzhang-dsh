...
`;
  ...
  // balance
  async function handleBalance(endpoint, payload, signal) {
    void payload;
    void signal;
  ...
  async function handlePricing(endpoint, payload, signal) {
    void payload;
    void signal;
  ...
  async function handleSummary(endpoint, payload, signal) {
    void payload;
    void signal;
  ...
  // today（今日消费）
  async function handleToday(endpoint, payload, signal) {
    void payload;
    void signal;
  ...
  // capabilities（探测各功能依赖的服务是否就绪，供前端优雅降级）
  async function handleCapabilities(endpoint, payload, signal) {
    void payload;
    void signal;
  ...
  // 单前缀 RPC 分发器：connection.rpc.handle 同一前缀只允许注册一次，
  // 故 5 个端点合并为一次 handle，按 endpoint 分发到上面的处理函数。
  const RPC_ENDPOINTS = {
  ...
  channel.handle(
    ...
    async (endpoint, payload, signal) => {
      const h = RPC_ENDPOINTS[endpoint];
      if (!h) return err("未知端点：" + endpoint, "bad-request");
      return h(endpoint, payload, signal);
    },
