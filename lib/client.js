...
export function apply(ctx) {
  ...
  function SuanzhangView(props) {
    ...
    const pricingTag = capabilities && capabilities.pricing === false
      ? "计价功能暂不可用"
      : pricing && pricing.source === "official"
        ? "官方价 · 自动同步 " + (pricing.fetchedAt ? fmtTime(pricing.fetchedAt) : "")
        : "内置价";
    ...
    return h(
      ...
      wide
        ? [
            ...
            balanceDown
              ? h("span", { className: "sz-sb-val warn" }, "功能暂不可用")
        ...
        : h("span", { className: "sz-sb-val" + (balanceDown || low ? " warn" : "") }, balanceDown ? "暂不可用" : value != null ? "¥" + value.toFixed(2) : "—"),
    );
  }
